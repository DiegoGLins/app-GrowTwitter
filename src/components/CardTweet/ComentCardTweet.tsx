...
const ComentCardTweet: React.FC<ComentCardTweetProps> = ({ avatarReTweet, nameUserReTweet }) => {
    ...
    return (
        <div style={{ display: 'flex', borderTop: '1.7px solid #e9e9e9', borderBottom: '1.7px solid #e9e9e9', borderBottomStyle: 'inset', width: '99%', padding: '6px 0px 0px 0px' }}>
            ...
            <p style={{ maxWidth: '250px', minHeight: '15px', padding: '2px 0px 0px 5px' }}><strong>{nameUserReTweet.length > 2 ? `${nameUserReTweet[0]} e mais ${nameUserReTweet.length - 1} pessoas` : nameUserReTweet.length > 1 ? `${nameUserReTweet[0]} e mais ${nameUserReTweet.length - 1} pessoa` : nameUserReTweet}</strong> </p>
